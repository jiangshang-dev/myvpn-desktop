...
1. 项目定位
本项目仅为计算机网络技术教学演示项目，**设计目标仅限局域网、企业内网环境内部通信，仅用于学习隧道、虚拟组网相关编程原理**。
本项目**不支持、不允许、不鼓励任何私自建立跨境网络信道的行为**，严禁用于绕过网络监管访问境外互联网。
**本项目开箱即用**

3. 使用强制约束
根据《计算机信息网络国际联网管理暂行规定》，在中国大陆境内，任何单位与个人不得私自搭建非合规信道进行国际联网。
- 如果你所在地区法律禁止本类技术，请勿下载、编译、运行本项目。
- 任何使用者自行承担全部使用风险，使用者必须保证自身使用行为完全符合所在地全部法律法规。
- 严禁将本项目代码、二次修改产物用于非法跨境联网、网络攻击、违规代理等一切违法场景。

3. 责任豁免
本软件按“现状”开源交付，不提供任何明示或默示担保。
项目作者、贡献者不对以下情形承担任何法律责任：
- 使用者私自修改代码，突破限制，将程序用于违法用途；
- 使用者部署、运行本软件产生的一切直接、间接损失，包括行政处罚、刑事追责、财产损失等；
- 第三方基于本项目衍生、二次开发出来的程序产生的全部后果。

4. 重要提示
即便有本免责声明，使用者依然需要自行遵守全部法律。**本声明不能免除使用者的法定责任。**
如果你无法满足上述约束，请不要使用本项目任何代码。
